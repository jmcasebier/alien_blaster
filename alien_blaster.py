...
#imports
import random, os.path, pygame, pickle
...
#get high score
def get_high_score():
    data = list(pickle.load(open(save_file, 'rb')))
    return data[0]
...
#pauses game
def pause(screen, background, high_score):
    ...
    score_text = pygame.font.SysFont(None, 24)
    score_text.set_bold(1)
    score_surface = score_text.render('High Score: ' + str(high_score), True,
    (100, 100, 100))
    score_rect = score_surface.get_rect()
    score_rect.centerx = (SCREEN_RECT.width / 2)
    score_rect.centery = (text_rect.centery * 1.25)
...
def main(windowstyle=0):
    ...
    while player.alive():
        ...
        for event in pygame.event.get():
            ...
            if event.type == KEYDOWN and event.key == K_p:
                PAUSED = True
                pause(screen, background, high_score)
